...
namespace Utils
{
                                                 ...
                                                 bool compute_in_2d)
  {
    ...
    GridGenerator::subdivided_hyper_rectangle(bulk_triangulation,
                                              ...
                                              Point<2>(left, 0.0),
                                              Point<2>(2.5, 0.41)); // changed from (2.2, 0.41) for Turek-Hron FSI benchmarking
        ...
        for (unsigned int f = 0; f < GeometryInfo<2>::faces_per_cell; ++f)
          {
            if (cell->face(f)->at_boundary())
              {
                if (std::abs(cell->face(f)->center()[0] - 2.5) < 1e-12) // changed from 2.2 for Turek-Hron FSI benchmarking
                  {
